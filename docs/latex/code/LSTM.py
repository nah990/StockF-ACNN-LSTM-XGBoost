...
plt.title('Остаток: Потери на обучающем и проверочном наборе')
plt.legend()
plt.show()

plt.figure(figsize=(10, 6))
plt.plot(yuan_history.history['loss'], label='Потери на обучающем наборе')
plt.plot(yuan_history.history['val_loss'], label='Потери на проверочном наборе')
plt.title('LSTM: Потери на обучающем и проверочном наборе')
plt.legend()
plt.show()
...
plt.plot(yuan_data.loc['2021-06-22':, 'close'], label='Реальный курс акций')
plt.plot(finalpredicted_stock_price['close'], label='Предсказанный курс акций')
plt.title('BiLSTM: Предсказание курса акций')
...
plt.figure(figsize=(10, 6))
plt.plot(yuan_real_stock_price1['close'], label='Реальный курс акций')
plt.plot(yuan_predicted_stock_price1['close'], label='Предсказанный курс акций')
plt.title('LSTM: Предсказание курса акций')
plt.xlabel('Дата', fontsize=12, verticalalignment='top')
plt.ylabel('Цена закрытия', fontsize=14, horizontalalignment='center')
plt.legend()
plt.show()
