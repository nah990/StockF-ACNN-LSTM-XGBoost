...
plt.plot(time, y, label='Остаток')
plt.plot(time, yhat, label='Предсказанный остаток')
plt.title('ARIMA+XGBoost: Предсказанный остаток')
plt.xlabel('Дата', fontsize=12, verticalalignment='top')
plt.ylabel('Остаток', fontsize=14, horizontalalignment='center')
plt.legend()
plt.show()
...
plt.figure(figsize=(10, 6))
plt.plot(time, data1, label='Реальный курс акций')
plt.plot(time, finalpredicted_stock_price, label='Предсказанный курс акций')
plt.title('ARIMA+XGBoost: Предсказание курса акций')
plt.xlabel('Дата', fontsize=12, verticalalignment='top')
